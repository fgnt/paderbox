...
class TestSegment(unittest.TestCase):
    ...
    def test_error_checking(self):
        ...
        self.assertRaises(ValueError,
                          lambda: segment_axis(np.arange(7), length=3,
                                               overlap=3))
        self.assertRaises(ValueError,
                          lambda: segment_axis(np.arange(7), length=8,
                                               overlap=3))
    ...
    def test_ending(self):
        ...
        assert_equal(
            segment_axis(np.arange(6), length=3, overlap=1, end='wrap'),
            np.array([[0, 1, 2], [2, 3, 4], [4, 5, 0]]))
        assert_equal(segment_axis(np.arange(6), length=3, overlap=1, end='pad',
                                  endvalue=-17),
    ...
    def test_multidimensional(self):
        ...
        assert_equal(
            segment_axis(np.ones((2, 3, 4, 5, 6)), axis=2, length=3, overlap=1,
                         end='cut').shape,
            (2, 3, 1, 3, 5, 6))
