...
def _merge_dicts(*dicts):
    ...
    # Duplicate test
    assert len(keys) == len(set(keys)), dicts
...
def to_rttm_str(data, smaple_rate=16000):
    """
    >>> ar1 = zeros(None)
    >>> ar1[0:16000] = 1
    >>> ar1[32000:48000] = 1
    ...
    SPEAKER S02 1 2 1 <NA> <NA> 1 <NA>
    SPEAKER S02 1 0 2 <NA> <NA> 2 <NA>
    >>> print(to_rttm_str(data, smaple_rate=320_000))
    ...
    for file_id in data.keys():
        for name in data[file_id].keys():
            content = data[file_id][name]
            if isinstance(content, np.ndarray):
                content = ArrayIntervall(content)
            for begin, end in content.intervals:
                duration = decimal.Decimal(end - begin) / smaple_rate
                begin = decimal.Decimal(begin) / smaple_rate
                lines.append(
